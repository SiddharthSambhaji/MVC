...
public class AuthService : System.Web.Services.WebService {
    ...
    /// <param name="lockoutRequired"></param>
    /// <returns></returns>
    [WebMethod]
    public async System.Threading.Tasks.Task<LoggedInUser> AuthenticateIdentityUser(ApplicationUserManager userManager, ApplicationSignInManager SignInManager, string UserName, string Password, bool RememberMe, bool lockoutRequired)
    {
        // WebUser user = UserManager.AuthenticateUser(userManager,SignInManager,UserName, Password, RememberMe, lockoutRequired);
        UserManager um = new UserManager();
        LoggedInUser loggedInUser = await um.AuthenticateUserfromIdentity(userManager, SignInManager, UserName, Password, RememberMe, lockoutRequired);
        if (loggedInUser != null)
        {
            ...
            //In order to track a user as logged onto the SSO site user token has to be presented in the cookie as well as 
            //he/she has to be presented in teh Application scope
            HttpContext.Current.Application[loggedInUser.Token] = loggedInUser;
        }
        return loggedInUser;
